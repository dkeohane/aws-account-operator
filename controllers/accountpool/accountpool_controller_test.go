...
func TestHSPool(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Test 2 Pools for Hypershift",
			localObjects: []runtime.Object{
				&awsv1alpha1.AccountPool{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "regularpool",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			assert.NoError(t, err, "Unexpected Error")
			al, err := getAccountPoolAccounts(mocks.fakeKubeClient, "regularpool", "aws-account-operator")
			assert.Len(t, al.Items, 1)
			assert.Equal(t, al.Items[0].Spec.AccountPool, "regularpool")
			...
			assert.NoError(t, err, "Unexpected Error")
			al, err = getAccountPoolAccounts(mocks.fakeKubeClient, "hspool", "aws-account-operator")
			assert.Len(t, al.Items, 1)
			assert.Equal(t, al.Items[0].Spec.AccountPool, "hspool")
		})
	}
}

func getAccountPoolAccounts(c client.Client, poolName string, namespace string) (awsv1alpha1.AccountList, error) {
	al := awsv1alpha1.AccountList{}
	err := c.List(context.TODO(), &al, client.InNamespace(namespace))

	poolAL := awsv1alpha1.AccountList{}
	for _, acc := range al.Items {
		if acc.Spec.AccountPool == poolName {
			poolAL.Items = append(poolAL.Items, acc)
		}
	}
	return poolAL, err
}
